...
namespace Capstone.HPTY.ServiceLayer.Services.HotpotService
{
    public class HotPotInventoryService : IHotPotInventoryService
    {
        ...
        public async Task<PagedResult<HotPotInventory>> GetPagedAsync(int pageNumber, int pageSize)
        {
            var query = _unitOfWork.Repository<HotPotInventory>()
                .FindAll(i => !i.IsDelete)
                .Include(i => i.Hotpot);
            ...
            var items = await query
                .OrderBy(i => i.HotPotInventoryId) // Ensure consistent ordering
                .Skip((pageNumber - 1) * pageSize)
                .Take(pageSize)
                .ToListAsync();

            return new PagedResult<HotPotInventory>
            {
                Items = items,
